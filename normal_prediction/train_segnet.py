...
from probe_network_segnet_double import ProbeNetwork
import datetime
...
def placeholder_inputs():
    ...
    groundtruth_ph_on = tf.placeholder(tf.int32, shape=(batch_size, 2*probe_num))
    return pointclouds_ph, probepoints_ph, groundtruth_ph, groundtruth_ph_on
...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(FLAGS.gpu)):
            ...
            pointclouds_ph, probepoints_ph, gt_ph, gt_ph_on = placeholder_inputs()
            ...
            seg_pred_on, seg_pred, feature = network.get_network_model(pointclouds_ph, probepoints_ph, \
            ...
            loss_on, seg_loss_on, per_instance_seg_loss_on, per_instance_seg_pred_res_on  \
                = network.get_loss(seg_pred_on, gt_ph_on)

            loss_all = loss + loss_on
            per_instance_seg_pred_res_all = tf.concat([per_instance_seg_pred_res, per_instance_seg_pred_res_on], axis=1)
            ...
            train_op = trainer.minimize(loss_on, var_list=train_variables, global_step=batch)
            ...
            iou = tf.metrics.mean_iou(gt_ph_on, per_instance_seg_pred_res_on, 50)
        ...
        #saver.restore(sess, 'train_results/2019_03_14_16_42_05/trained_models/model.ckpt')
        ...
        def train_one_epoch(epoch_num):
            ...
            train_generator = pv.provide_seg_data_train(batch_size, point_num, probe_num)

            printout(flog, 'train_one_epoch*****************************************')
            total_loss = 0.0
            total_seg_loss = 0.0
            total_seg_acc = 0.0
            total_seg_acc_on = 0.0
            num_batch = 0
            ...
            for data in train_generator:
                ...
                _, loss_val, seg_loss_val, \
                        per_instance_seg_loss_val, seg_pred_val, pred_seg_res,pred_seg_res_on, \
                        = sess.run([train_op, loss_all, seg_loss, \
                        per_instance_seg_loss, seg_pred, per_instance_seg_pred_res, per_instance_seg_pred_res_on], \
                        feed_dict=feed_dict)

                per_instance_part_acc = np.mean(pred_seg_res == data[6], axis=1)
                per_instance_part_acc_on = np.mean(pred_seg_res_on == data[7], axis=1)
                average_part_acc = np.mean(per_instance_part_acc)
                average_part_acc_on = np.mean(per_instance_part_acc_on)

                total_loss += loss_val
                total_seg_loss += seg_loss_val

                total_seg_acc += average_part_acc
                total_seg_acc_on += average_part_acc_on

            total_loss = total_loss * 1.0 / num_batch
            total_seg_acc_on = total_seg_acc_on * 1.0 / num_batch
            ...
            printout(flog, '\t\tTraining Seg Accuracy points_on: %f' % total_seg_acc_on)

        def eval_one_epoch(epoch_num):
            is_training = False
            train_generator = pv.provide_seg_data(is_training, batch_size, point_num, probe_num)
        ...
        def eval_one_epoch_on(epoch_num):
            is_training = False
            ...
            for data in train_generator:
                ...
                # print ("-----------------------")
                loss_val, seg_loss_val, \
                        per_instance_seg_loss_val, seg_pred_val, pred_seg_res_on \
                        = sess.run([loss_on, seg_loss_on, \
                        per_instance_seg_loss_on, seg_pred_on, per_instance_seg_pred_res], \
                        feed_dict=feed_dict)

                per_instance_part_acc = np.mean(pred_seg_res_on == data[7], axis=1)
                average_part_acc = np.mean(per_instance_part_acc)
        ...
        def eval_one_epoch_iou(epoch_num):
            is_training = False
            train_generator = pv.provide_seg_data_test(batch_size, point_num, probe_num)
            printout(flog, 'test_one_epoch*****************************************')
            total_loss = 0.0
            ...
            total_seg_acc = 0.0
            total_iou = 0.0
            total_confuse_matrix = np.zeros(shape=(50, 50))
            num_batch = 0
            ...
            for data in train_generator:
                ...
                # print ("-----------------------")
                loss_val, seg_loss_val, \
                        per_instance_seg_loss_val, seg_pred_val, pred_seg_res, test_iou \
                        = sess.run([loss, seg_loss, \
                        per_instance_seg_loss, seg_pred, per_instance_seg_pred_res_on, iou], \
                        feed_dict=feed_dict)

                per_instance_part_acc = np.mean(pred_seg_res == data[7], axis=1)
                average_part_acc = np.mean(per_instance_part_acc)
            ...
            total_seg_loss = total_seg_loss * 1.0 / num_batch
            total_seg_acc = total_seg_acc * 1.0 / num_batch
            total_iou = total_iou * 1.0 / (num_batch-1)
            ...
            printout(flog, '\t\tTesting Seg Mean_loss: %f' % total_seg_loss)
            printout(flog, '\t\tTesting Seg Accuracy: %f' % total_seg_acc)
            printout(flog, '\t\tTesting mIoU: %f' % test_iou[0])
            myiou = np.mean(np.trace(test_iou[1]) / sum((np.sum(test_iou[1], axis=0) + np.sum(test_iou[1], axis=1) - np.diag(test_iou[1]))))
            printout(flog, '\t\tTesting mIoU: %f' % myiou)
            return total_seg_acc
