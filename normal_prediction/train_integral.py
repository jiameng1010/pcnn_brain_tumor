...
parser.add_argument('--point_num', type=int, default=625, help='Point Number [256/512/1024/2048]')
...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(FLAGS.gpu)):
            ...
            normal_pred = tf.gradients(seg_pred, probepoints_ph)[0]
            pred = tf.reduce_sum(normal_pred, axis=2)
            loss, seg_loss, per_instance_seg_loss, per_instance_seg_pred_res  \
                = network.get_loss(pred, gt_ph)
