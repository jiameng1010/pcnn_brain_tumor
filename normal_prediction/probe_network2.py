...
class ProbeNetwork:
    ...
    def get_network_model(self, pointclouds_pl,probepoints, \
        ...
        ps_function_pl = tf.concat([pointclouds_pl, tf.ones(shape=[batch_size, num_point, 1], dtype=tf.float32)],
                                   axis=2)
        probepoints_pl = get_probepoints(batch_size, 10)
        ...
        for block_index, block in enumerate(blocks):
            ...
            for out_index, out_channel in enumerate(block):
                ...
                _, network_probe_real = convlayer.get_layer(
                    probepoints, network, with_bn, bn_decay,False,True)
                probe.append(network_probe_real)
                network, network_probe = convlayer.get_layer(
                    probepoints_pl, network, with_bn, bn_decay,False,True)

                input_channel = out_channel


            pointclouds_pl_down.append([tf.concat([probepoints_pl,pointclouds_pl], axis=1), tf.concat([network_probe,network], axis=1)])
        ...
        pointclouds_pl_down.append([tf.concat([probepoints_pl,pointclouds_pl], axis=1), None])
        ...
        for i in range(len(pointclouds_pl_down) -1 ,0,-1):
            if i == 3:
                deconvnet = DeconvLayer(pointclouds_pl_down[i][0],tf.concat([network_probe,network], axis=1),pointclouds_pl_down[i-1][0]).get_layer()
            else:
        ...
        network_probe = tf.layers.conv1d(network_probe_P, 2, 1)
    ...
    def get_normal_loss(self, normal_pred, normal):
        ...
        return tf.losses.cosine_distance(normal, normal_pred_norm, axis=-1), tf.losses.mean_squared_error(normal, normal_pred_norm)
