...
from probe_network_no_grid_sigma import ProbeNetwork
import datetime
...
parser.add_argument('--config', type=str, default='pointconv_probe_braintumor', help='Config file name')
...
parser.add_argument('--sigma', type=float, default=3.0, help='sigma (radius) of extention operator')
...
# MAIN SCRIPT
point_num = cfg['training']['num_points']
...
os.system("""cp {0} "{1}" """.format('../confs/training.conf', MODEL_STORAGE_PATH))
...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(FLAGS.gpu)):
            ...
            pointlabel_ph = tf.placeholder(tf.uint8, shape=(batch_size, point_num))
            ...
            disp_pred, feature = network.get_network_model(pointclouds_ph, probepoints_ph, pointlabel_ph, extend_ph, extend_factor=Sigma,\
            ...
            #per_point_loss, per_instance_loss, total_loss_total = network.get_loss_displacement_abs(disp_pred, gt_ph)
            total_loss_total, total_metric = network.get_loss_displacement(disp_pred, gt_ph)
            ...
            train_op = trainer.minimize(total_loss_total, var_list=train_variables, global_step=batch)
        ...
        def train_one_epoch(epoch_num):
            is_training = True
            train_generator = pv.provide_data_edma(is_training, batch_size, point_num, probe_num)
            printout(flog, 'train_one_epoch*****************************************')
            total_loss_train = 0.0
            total_metric_train = 0.0
            num_batch = 0
            ...
            for data in train_generator:
                ...
                feed_dict = {
                        ...
                        probepoints_ph: data[2],
                        gt_ph: data[3],
                        pointlabel_ph: data[4],
                        extend_ph: data[5],
                ...
                #
                # print ("-----------------------")
                _, loss_train, metric_train = sess.run([train_op, total_loss_total, total_metric], feed_dict=feed_dict)

                total_loss_train += loss_train
                total_metric_train += metric_train

            total_loss_train = total_loss_train * 1.0 / (num_batch * batch_size)
            total_metric_train = total_metric_train * 1.0 / (num_batch * batch_size)

            printout(flog, '\tTraining Mean Loss: %f' % total_loss_train)
            printout(flog, '\tTraining Mean Error: %f' % total_metric_train)
        ...
        def train_one_epoch_normal(epoch_num):
            ...
            if (epoch_num%2 == 0):
                ...
                train_generator = pv.provide_modelnet_data(is_training, batch_size, point_num, probe_num)
            printout(flog, 'train_normal_one_epoch*****************************************')
            total_loss = 0.0
            total_metric = 0.0
        ...
        def eval_one_epoch(epoch_num):
            ...
            for data in train_generator:
                ...
                #
                # print ("-----------------------")
                _, loss_val, metric_val = sess.run([train_op, total_loss_total, total_metric], feed_dict=feed_dict)

                total_loss_val += loss_val
                total_metric_val += metric_val

            total_loss_val = total_loss_val * 1.0 / (num_batch * batch_size)
            total_metric_val = total_metric_val * 1.0 / (num_batch * batch_size)

            printout(flog, '\tTest Mean Loss: %f' % total_loss_val)
            printout(flog, '\tTest Mean Error: %f' % total_metric_val)

            return total_metric_val

        def visualize(output_folder):
            output_dir = os.path.join(output_folder, 'visualize')
            if not os.path.exists(output_dir):
                os.mkdir(output_dir)
            is_training = False
            visual_generator = pv.provide_data(is_training, batch_size, point_num, probe_num, shuffle=False)
            num = 0
            for data in visual_generator:
                generator = octree_generator(pointcloud=data[0],  # +np.random.normal(scale=0.05, size=data[0].shape),
                                             sess=sess,
                                             tensors=[pointclouds_ph, probepoints_ph, is_training_ph, seg_pred, normal_pred,
                                                      feature],
                                             random_surface_points=None,)
                mesh = generator.save_mesh(output_dir+'/'+str(num).zfill(3)+'.off')
                num += 1
                if num > 10:
                    break
        ...
        for epoch in range(TRAINING_EPOCHES):
            ...
            if (acc < acc_min):
                ...
                #visualize(os.path.join(MODEL_STORAGE_PATH,'epoch_{0}'.format(epoch+1)))
        ...
        # save global resutle
        fout_global = open(FLAGS.output_dir+'/radius_inv_factor.txt', 'r')
        results = fout_global.readlines()
        fout_global.close()
        fout_global = open(FLAGS.output_dir+'/radius_inv_factor.txt', 'w')
        for item in results:
            fout_global.write(item)
        fout_global.write('sigma=' + str(Sigma) + ': ' + 'accuracy ' + str(acc_min) + '   ' + MODEL_STORAGE_PATH + '\n')
        fout_global.close()
