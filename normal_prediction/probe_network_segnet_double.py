...
class ProbeNetwork:
    ...
    def get_network_model(self, pointclouds_pl,probepoints, \
        ...
        block_elm = ConvElements(probepoints, pointclouds_pl_down[0][0],tf.reciprocal(tf.sqrt(1.0 * pointclouds_pl_down[0][0].get_shape()[1].value)),
                                      spacing,
                                      self.conf.get_float('kernel_sigma_factor')) \

        network, network_probe = ConvLayer(network.get_shape()[-1].value, block_elm,
                                 50, "{0}_deconv".format('last'),
                           is_training).get_layer(probepoints, network, False, bn_decay,False ,False)
        probe.append(network_probe)

        network_probe_P = tf.concat(probe, axis=2)
        network_probe = tf.layers.conv1d(network_probe_P, 512, 1, activation=tf.nn.relu)
        network_probe = tf.layers.conv1d(network_probe, 50, 1)

        return network, network_probe, network_probe_P
