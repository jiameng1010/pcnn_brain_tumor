...
class ProbeNetwork:
    ...
    def get_network_model(self, pointclouds_pl,probepoints, \
        ...
        network_probe_P = tf.reshape(network_probe_P, [batch_size, num_probe, 256])
        #network_probe_P = tf.layers.conv1d(network_probe_P, 128, 1)
        network_probe = tf.layers.dense(network_probe_P, 1)'''
        network_probe = tf.layers.conv1d(network_probe_P, 2, 1)

        return network_probe, network_probe_P
        
    def get_loss_logistic(self, seg_pred, seg):
        per_instance_seg_loss = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(logits=tf.squeeze(seg_pred), labels=tf.to_float(seg)), axis=1)
        seg_loss = tf.reduce_mean(per_instance_seg_loss)
        per_instance_seg_pred_res = tf.math.greater(seg_pred, tf.zeros_like(seg_pred))
        per_instance_seg_pred_res = tf.squeeze(tf.dtypes.cast(per_instance_seg_pred_res, dtype=tf.int16))
        total_loss =  seg_loss

        return total_loss, seg_loss, per_instance_seg_loss, per_instance_seg_pred_res
    
    def get_loss_hinge(self, seg_pred, seg):
        per_instance_seg_loss = tf.reduce_mean(
            tf.losses.hinge_loss(logits=tf.squeeze(0.5*seg_pred), labels=tf.to_float(seg), reduction=tf.losses.Reduction.NONE), axis=1)
        seg_loss = tf.reduce_mean(per_instance_seg_loss)
        per_instance_seg_pred_res = tf.math.greater(seg_pred, tf.zeros_like(seg_pred))
        per_instance_seg_pred_res = tf.squeeze(tf.dtypes.cast(per_instance_seg_pred_res, dtype=tf.int16))
        total_loss =  seg_loss
