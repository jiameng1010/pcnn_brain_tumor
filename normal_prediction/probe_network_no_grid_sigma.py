...
from convlayer_elements_sigma import ConvElements, DeConvElements
import tf_util
...
class ProbeNetwork:
    ...
    def get_network_model(self, pointclouds_pl,probepoints, pointlabel_ph, extends_ph, extend_factor,\
        ...
        extends_mean = tf.reduce_mean(extends_ph, axis=1)
        #extends_mean = tf.exp(extends_mean)

        for block_index, block in enumerate(blocks):
            block_elm = ConvElements(probepoints, pointclouds_pl, extend_factor*extends_mean/np.sqrt(pointclouds_pl.get_shape()[1].value,dtype=np.float32), spacing,
            ...
            for out_index, out_channel in enumerate(block):
                ...
                network, network_probe_real = convlayer.get_layer(
                    probepoints, network, with_bn, bn_decay,False,True,False)
        ...
        for i in range(len(pointclouds_pl_down) -1 ,0,-1):
            ...
            block_elm = ConvElements(probepoints, pointclouds_pl_down[i - 1][0],
                              extend_factor*extends_mean/np.sqrt(pointclouds_pl_down[i - 1][0].get_shape()[1].value, dtype=np.float32), spacing,
            ...
            convnet, network_probe = ConvLayer(deconvnet.get_shape()[-1].value, block_elm, pointclouds_pl_down[i-1][1].get_shape()[-1].value, "{0}_deconv".format(i),
                           is_training).get_layer(probepoints,deconvnet, with_bn , bn_decay, False,True,False)
        ...
        #network_probe = extend_factor * extend_factor *\
        #                tf.expand_dims(tf.expand_dims(extends_mean * extends_mean, axis=1), axis=2) * \
        #                tf.layers.conv1d(network_probe_P, 3, 1, use_bias=False)
        network_probe = tf.layers.conv1d(network_probe_P, 3, 1, use_bias=False)

        return network_probe, network_probe_P

    def get_error_displacement(self, disp_pred, disp):
        error = disp_pred - disp
        sq_error = error * error
        per_point_error = tf.reduce_sum(sq_error, axis=2)
        per_point_error = tf.sqrt(per_point_error)
        total_error = tf.reduce_mean(per_point_error)
        return total_error

    def get_loss_displacement(self, disp_pred,  disp):
        huber_delta = self.conf.get_float('huber_delta')
        #loss =  tf.keras.losses.Huber(huber_delta)
        #loss1 = (1.0/huber_delta) * loss(disp_pred, disp)
        loss = tf.keras.losses.MeanAbsoluteError()
        loss1 = loss(disp_pred, disp)

        loss2 = -tf.reduce_sum(disp*disp_pred, axis=2)
        loss2 = loss2 / (tf.sqrt(tf.reduce_sum(disp_pred*disp_pred, axis=2)))
        loss2 = tf.reduce_mean(loss2)
        return loss1, 0.25709338473880267*self.get_error_displacement(disp_pred,  disp)
        '''error = disp_pred - disp
