...
class ProbeNetwork:
    ...
    def get_network_model(self, pointclouds_pl,probepoints_pl, \
        ...
        for block_index, block in enumerate(blocks):
            block_elm = ConvElements(probepoints_pl, pointclouds_pl, np.float32(1.)/np.sqrt(pointclouds_pl.get_shape()[1].value,dtype=np.float32), spacing,
                                          np.float32(self.conf.get_float('kernel_sigma_factor')))
            for out_index, out_channel in enumerate(block):
                network, network_probe = ConvLayer(input_channel, block_elm, out_channel,
                                     '{0}_block_{1}'.format(block_index, out_index), is_training).get_layer(
                    probepoints_pl, network, with_bn, bn_decay,False,True)

                input_channel = out_channel

            #pointclouds_pl_down.append([pointclouds_pl, network, probepoints_pl, network_probe])
            ...
            #probepoints_pl, network_probe = PoolingLayer(block_elm, out_channel, out_channel,
            #                                       int(pool_sizes_sigma[block_index + 1][0])).get_layer_probe(network_probe,use_fps = tf.constant(False),is_subsampling = self.conf.get_bool("is_subsampling"))

        pointclouds_pl_down.append([pointclouds_pl,None])
        #pointclouds_pl_down.append([probepoints_pl, None])
        ...
        for i in range(len(pointclouds_pl_down) -1 ,0,-1):
            ...
            block_elm = ConvElements(probepoints_pl, pointclouds_pl_down[i - 1][0],
            ...
            convnet, network_probe = ConvLayer(deconvnet.get_shape()[-1].value, block_elm, pointclouds_pl_down[i-1][1].get_shape()[-1].value, "{0}_deconv".format(i),
                           is_training).get_layer(probepoints_pl,deconvnet, with_bn , bn_decay, False,True)
        ...
        #block_elm = DeConvElements(pointclouds_pl_down[0][0],tf.reciprocal(tf.sqrt(1.0 * pointclouds_pl_down[0][0].get_shape()[1].value)),
        #                              spacing,
        #                              self.conf.get_float('kernel_sigma_factor')) \

        #network_probe = ConvLayer(network_probe.get_shape()[-1].value, block_elm,
        #                         2, "{0}_deconv".format('last'),
        #                   is_training).get_layer_org(network_probe, False, bn_decay,False ,False)
        network_probe = tf.concat(probe, axis=2)
