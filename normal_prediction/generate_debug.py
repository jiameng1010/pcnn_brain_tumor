...
visualize_dir = '/tmp/pycharm_pcnn_liver/normal_prediction/train_results/2020_07_07_01_26_53/trained_models/epoch_26'
...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(FLAGS.gpu)):
            ...
            #soft = tf.slice(soft, [0, 0, 0], [batch_size, probe_num, 1])
            soft = tf.slice(soft, [0, 0, 0], [batch_size, probe_num, 1]) - tf.slice(soft, [0, 0, 1], [batch_size, probe_num, 1])
            #soft = tf.math.sigmoid(seg_pred)
            ...
            #laplacian = tf.squeeze(tf.reduce_sum(normal_pred, axis=-1))
        ...
        sess.run(init)
        saver.restore(sess, visualize_dir+'/model.ckpt')
        ...
        def train_one_epoch(epoch_num):
            ...
            for data in train_generator:
                ...
                per_instance_part_acc = np.mean(pred_seg_res == data[3], axis=1)
        ...
        def eval_one_epoch(epoch_num):
            ...
            for data in train_generator:
                ...
                #
                # print ("-----------------------")
                loss_val, seg_loss_val, per_instance_seg_loss_val,\
                seg_pred_val, pred_seg_res, normal_vel \
                        = sess.run([loss, seg_loss, per_instance_seg_loss,\
                                    seg_pred, per_instance_seg_pred_res, normal_pred], \
                        feed_dict=feed_dict)
