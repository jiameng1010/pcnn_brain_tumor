...
np.random.seed(0)
from pointnormalcloud_conv_net import PointNormal_Network
...
def train():
    ...
    with tf.Graph().as_default():
        ...
        with tf.device('/gpu:' + str(GPU_INDEX)):
            ...
            surfacenormal_pl = tf.placeholder(tf.float32, shape=(BATCH_SIZE,NUM_POINT, 3))
            ...
            network = PointNormal_Network(conf.get_config('pn_network'))
            pred, mean = network.build_network(pointclouds_pl,surfacenormal_pl,is_training_pl,is_evaluate_pl,bn_decay)
        ...
        saver.restore(sess, "/home/mjia/Documents/jointSegmentation/pcnn/exp_results/pcnn/monavale/records/2019_01_07_03_06_59/epoch_150/model.ckpt")
        print(np.mean(np.abs(sess.run(tf.trainable_variables()[6]))))
        ops = {'pointclouds_pl': pointclouds_pl,
               'surfacenormal_pl': surfacenormal_pl,
               'labels_pl': labels_pl,
        ...
        def train_one_epoch(sess, ops, train_writer,epoch):
            ...
            for fn in range(len(TRAIN_FILES)):
                log_string('----' + str(fn) + '-----')
                current_data, current_normal, current_label = provider.loadDataFile_pn(TRAIN_FILES[train_file_idxs[fn]])

                current_data = current_data[:, : , :]
                current_normal = current_normal[:,:,:]
                current_data, current_normal, current_label, idx = provider.shuffle_data_pn(current_data, current_normal, np.squeeze(current_label))
                ...
                for batch_idx in range(num_batches):
                    ...
                    if (conf.get_bool('pn_network.with_rotations')):
                        ...
                        random_choice_of_points = np.random.choice(np.arange(conf.get_int('no_points_sample')), NUM_POINT, False)
                        augmented_data, augmented_normal = provider.translate_point_cloud_pn(current_data[start_idx:end_idx, random_choice_of_points, :], \
                                                                                             current_normal[start_idx:end_idx, random_choice_of_points, :])

                    feed_dict = {ops['pointclouds_pl']: augmented_data,
                                 ops['labels_pl']: current_label[start_idx:end_idx],
                                 ops['is_training_pl']: is_training,
                                 ops['is_evaluate_pl']: is_evaluate,
                                 ops['surfacenormal_pl']: augmented_normal}

                    summary, step, _, loss_val, pred_val, mean = sess.run([ops['merged'], ops['step'],
                                                                     ops['train_op'], ops['loss'], ops['pred'], ops['mean']],
                                                                    feed_dict=feed_dict)
                    #print(mean)
        ...
        def eval_one_epoch(sess, ops, test_writer, epoch_index):
            global total_paramers
            ...
            for fn in range(len(TEST_FILES)):
                log_string('----' + str(fn) + '-----')
                current_data, current_normal, current_label = provider.loadDataFile_pn(TEST_FILES[fn])
                ...
                for batch_idx in range(num_batches):
                    ...
                    if (cur_batch_size < BATCH_SIZE):
                        ...
                        current_normal_feed = current_normal[start_idx:end_idx, :, :]
                        #current_data_feed, current_normal_feed = provider.translate_point_cloud_pn(current_data[start_idx:end_idx, :, :], \
                        #                                                                     current_normal[start_idx:end_idx, :, :])

                    feed_dict = {ops['pointclouds_pl']: current_data_feed,
                                 ops['labels_pl']: current_label_feed,
                                 ops['is_training_pl']: is_training,
                                 ops['is_evaluate_pl']: is_evaluate,
                                 ops['surfacenormal_pl']: current_normal_feed}
            ...
            log_string(('eval accuracy: %f' % (total_correct / float(total_seen))) + (' max: %f'% (max_accu)))
        ...
        for epoch in range(MAX_EPOCH):
            ...
            acc = eval_one_epoch(sess, ops, test_writer, epoch)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='confs/var_lesspoints_2.conf', help='Config to use [default: pointconv]')
    ...
    os.system("""cp {0} "{1}" """.format('pointcloud_conv_net_pn.py', LOG_DIR))
    ...
    os.system("""cp train_pn.py "{0}" """.format(LOG_DIR))
    ...
    NUM_POINT = conf.get_list('pn_network.pool_sizes_sigma')[0][0]
