...
class ConvLayer_pn:
    ...
    def get_layer(self, functions_pl, with_bn, bn_decay,interpolation,with_Relu = True):
        ...
        with tf.variable_scope(self.scope) as sc:
            ...
            outputs2 = tf.slice(outputs, [0,0,0], [self.block_elemnts.batch_size, self.block_elemnts.num_of_points, self.out_channels])
            #return outputs, tf.reduce_sum(tf.to_float(tf.greater(outputs2, tf.zeros_like(outputs2))))/tf.reduce_sum(tf.ones_like(outputs2))
            return outputs, tf.reduce_sum(tf.to_float(outputs2))
    ...
    def get_convlution_operator_2(self,functions_pl,interpolation,dtype=tf.float32,type=1):
        ...
        def convopeator_per_translation(input):
            ...
            #           - (((gamma**2)*(alpha**2))*b2 + (self.block_elemnts.sigma_f)**2) / (2*((gamma**2)+(alpha**2))*((self.block_elemnts.sigma_f)**2))
            q_tensor = - (((gamma**2)*(beta**2)) / (2*((gamma**2)+(beta**2))*((self.block_elemnts.sigma_f)**2)))*a2\
            ...
            if type == 1:
                #q_tensor = tf.multiply(b, q_tensor)
                factor = self.block_elemnts.sigma_f * (gamma**2) * np.sqrt(2 * np.pi) * b / np.power((alpha**2+gamma**2), 1.5)
                q_tensor = factor * q_tensor
            elif type ==2:
                aa = (alpha**2+gamma**2) / (2 * self.block_elemnts.sigma_f**2)
                ...
                aa = (alpha**2+gamma**2) / (2 * self.block_elemnts.sigma_f**2)
                bb = (gamma**2 / self.block_elemnts.sigma_f**2) * b / self.block_elemnts.sigma_f**2
                ...
                bc2a = (gamma**2 * ci / (self.block_elemnts.sigma_f**2 * (alpha**2+gamma**2))) * b
                factor = 2 * np.pi * (self.block_elemnts.sigma_f**3) * np.sqrt(np.pi / (alpha**2+gamma**2)) / (beta**2+gamma**2)
                q_tensor = factor * q_tensor * tf.sin(bc2a)
