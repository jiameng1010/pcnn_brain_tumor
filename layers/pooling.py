...
class PoolingLayer:
    ...
    def get_layer_probe(self, network,use_fps,startidx = None,is_subsampling = False):
        batch_size = self.block_elm.batch_size
        distances_probe = tf.expand_dims(self.block_elm.probe_points, axis=2) - tf.expand_dims(self.block_elm.probe_points, axis=1)
        distances_probe = tf.reduce_sum(tf.multiply(distances_probe, distances_probe), axis=3)
        idx = tf.cond(use_fps,lambda:self.fps(distances_probe,startidx),lambda:tf.tile(tf.expand_dims(tf.range(self.k),0),[batch_size,1]))
        temp = tf.expand_dims(tf.range(batch_size, dtype=tf.int32), axis=1)
        mask = tf.reshape(tf.stack([tf.reshape(
            tf.tile(temp, [1, idx.get_shape()[-1].value]),
            shape=[-1]), tf.reshape(tf.cast(idx, tf.int32), shape=[-1])], axis=1), shape=[batch_size, -1, 2])
        pooled_points_pl = tf.gather_nd(self.block_elm.probe_points,mask)
        if (is_subsampling):
            return pooled_points_pl,tf.gather_nd(network,mask)
        else:

            center_indexes = tf.argmin(tf.gather_nd(distances_probe,mask),axis=1)
