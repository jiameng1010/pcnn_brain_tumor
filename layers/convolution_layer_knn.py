...
class ConvLayer:
    def __init__(self, input_channels, block_elemnts, out_channels,scope,is_training,use_xavier=True,l2_regularizer = 1.0e-3):
        ...
        self.knn_k = int(self.block_elemnts.num_of_points/self.block_elemnts.knn_portion)
    ...
    def get_convlution_operator(self,functions_pl,interpolation,dtype=tf.float32):
        ...
        feature_distance = tf_util.pairwise_distance(functions_pl)
        #nn_idx = tf_util.knn(feature_distance, k=self.knn_k)
        #edge_features = tf_util.get_edge_feature(functions_pl, nn_idx=nn_idx, k=self.knn_k)
        ...
        b_tensor = tf.transpose(tf.matmul(tf.tile(tf.expand_dims(w_tensor, axis=1), [1, int(self.out_channels), 1, 1]),
        ...
        def convopeator_per_translation(input):
            ...
            nn_index = nn_idx
            b = tf_util.get_edge_feature(b_per_translation, nn_idx=nn_index, k=self.knn_k)
            ...
            distances_knn = tf_util.gather_distance(distances, nn_index)
            dot_knn = tf_util.gather_distance(tf.add(dot, -tf.transpose(dot, [0, 2, 1])), nn_index)
            q_tensor = tf.exp(-(tf.add(tf.add(distances_knn,
                                              dot_knn),
                              ...
                              #/ (20 * tf.reduce_max(distances_knn, axis=2, keepdims=True) ** 2))
            output = tf.squeeze(tf.matmul(tf.expand_dims(q_tensor, axis=-2), b))
            return output

        def convopeator_per_translation_org(input):
            translation_index = input[1]
            b_per_translation = input[0]
            ...
            q_tensor = tf.exp(-(tf.add(tf.add(distances,
                                       ...
                                       tf.expand_dims(tf.slice(translations_square, [0, translation_index],
                                                               [self.block_elemnts.batch_size, 1]), axis=2)))
                              / (20 * self.block_elemnts.combined_sigma ** 2))
