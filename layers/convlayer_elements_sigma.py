...
class ConvElements:
    def __init__(self, probe_points, points_pl ,sigma,spacing,kernel_sigma_factor):
        ...
        self.num_of_probe_points = self.probe_points.get_shape()[1].value

    def get_kernel_translations(self,spacing):
        def kernel_translation_per_instance(input):
            sigma_one = input[0]
            grid = tf.linspace(-spacing * sigma_one, spacing * sigma_one, tf.constant(3))
            return tf.reshape(tf.transpose(tf.reshape(tf.stack(tf.meshgrid(grid, grid, grid)), [3, -1])),
                              [self.num_of_translations, 3])

        return tf.stack(tf.map_fn(kernel_translation_per_instance,
                   elems=(self.sigma_k, tf.range(0, self.batch_size)),
                   dtype=tf.float32))
    ...
    def get_interpolation_matrix(self):
        return self.get_distance_matrix() / (2.0 * tf.expand_dims(tf.expand_dims(self.sigma_f*self.sigma_f, axis=-1), axis=-1))
